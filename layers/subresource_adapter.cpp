...
ImageRangeGenerator::ImageRangeGenerator(const ImageRangeEncoder& encoder, const VkImageSubresourceRange& subres_range,
    ...
    if (ExtentIsEmpty(extent) || ((extent.width + offset.x) > subres_extent.width) ||
        ((extent.height + offset.y) > subres_extent.height) || ((extent.depth + offset.z) > subres_extent.depth)) {
        // Empty range forces empty position -- no operations other than deref for empty check are valid
        pos_ = {0, 0};
        return;
    }
...
void ImageRangeGenerator::SetUpSubresIncrementer() {
    ...
    bool linear_image = encoder_->IsLinearImage();
    bool layers_interleave = linear_image && (subres_info_->layout.arrayPitch > subres_info_->layout.size);
    ...
    } else if (CoversAllLayers(full_range, subres_range_)) {
        if (!linear_image) {
